...
class LarmorPyPulseq(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        ...
        self.rfExTime = None
        self.nScans = None
        self.addParameter(key='seqName', string='LarmorInfo', val='rf_pulse_test')
        ...
        self.addParameter(key='rfExTime', string='RF excitation time (us)', val=15000.0, field='RF', units=units.us)
        self.addParameter(key='repetitionTime', string='Repetition time (ms)', val=50, field='SEQ', units=units.ms)
        self.addParameter(key='bw_rx', string='Rx bandwidth (kHz)', val=50, field='RF', units=units.kHz)
        self.addParameter(key='pulse_type', string='Pulse type', val='wurst', field='RF', tip="`wurst` or `hypsec`")
        self.addParameter(key='bw_tx', string='Tx bandwidth (kHz)', val=10, field='RF', units=units.kHz)
        self.addParameter(key='adiabaticity', string='Adiabaticity', val=4, field='RF')
    ...
    def sequenceInfo(self):
        print("Adiabatic Pulse Test")
        print("Author: PhD. J.M. Algarín")
        print("Contact: josalggui@i3m.upv.es")
        print("mriLab @ i3M, CSIC, Spain")
        print("This sequence runs an adiabatic inversion pulse and run the readout simultaneously\n")
        print("Connect Tx0->Rx0 to measure the pulse with the red pitaya")
