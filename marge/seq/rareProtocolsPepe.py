...
class RAREProtocolsPepe(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(RAREProtocolsPepe, self).__init__()
        # Input the parameters
        self.addParameter(key='seqName', string='RAREInfo', val='RAREProtocolsPepe')
        ...
        self.addParameter(key='Gdiff', string='Grad difussion (mT/m)', val=5.0, field='OTH')
    ...
    def sequenceRun(self, plotSeq=0, demo=False):
        ...
        # Difussion
        if self.Gdiff>0:
            self.mapVals['Gdiff'] = self.Gdiff
            self.mapVals['Tdiff'] = self.Tdiff
            self.mapVals['b_diff'] = ((hw.gammaB*self.Gdiff*self.Tdiff*1e-6)**2 * (self.echoSpacing - self.Tdiff/3)*1e-3)*1e-6 #s/mm²
            print("b-value = ", self.mapVals['b_diff'], " s/mm²")
            print("b=. %i" % self.mapVals['b_diff'])
            ...
            while acqPoints+self.etl*nRD<=hw.maxRdPoints and orders<=hw.maxOrders and repeIndexGlobal<nRepetitions:
                ...
                if (repeIndex==(self.dummyPulses-1)  or repeIndex>=self.dummyPulses) and dc==False:
                    if self.Gdiff  > 0:
                        ...
                        self.gradTrap(t0, gradRiseTime, self.mapVals['Tdiff']*1e3, self.Gdiff, gSteps, self.axesOrientation[0], self.shimming)
                        self.gradTrap(t0, gradRiseTime, self.mapVals['Tdiff']*1e3, self.Gdiff, gSteps, self.axesOrientation[1], self.shimming)
                        self.gradTrap(t0, gradRiseTime, self.mapVals['Tdiff']*1e3, self.Gdiff, gSteps, self.axesOrientation[2], self.shimming)
                        orders = orders+gSteps*6

                        t0 = tEx+self.rfExTime/2-hw.gradDelay + 2*gradRiseTime + self.mapVals['Tdiff']*1e3 + 500
