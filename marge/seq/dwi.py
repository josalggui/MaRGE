...
@Summary: DWIrare sequence class
"""
import os
import sys
#*****************************************************************************
...
class DWI(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(DWI, self).__init__()
        # Input the parameters
        self.addParameter(key='seqName', string='DWInfo', val='DWI')
        ...
        self.addParameter(key='rfExAmp', string='Excitation RF amplitude (a.u.)', val=0.1, field='RF')
        self.addParameter(key='rfReAmp', string='Refocusing RF amplitude (a.u.)', val=0.2, field='RF')
        ...
        self.addParameter(key='repetitionTime', string='Repetition time (ms)', val=1200., field='SEQ')
        ...
        self.addParameter(key='Tdiff', string='Time difussion (ms)', val=20.0, field='OTH')
        self.addParameter(key='tGapDiff2Deph', string='Time gap Gdiff-Gdeph (ms)', val=1.0, field='OTH')
        self.addParameter(key='DiffDirection', string='Difussion direction (x,y,z)', val=[0, 1, 0], field='OTH')
        self.addParameter(key='bValue', string='b-value (s/mm2)', val=500.0, field='OTH')
    ...
    def sequenceAtributes(self):
        ...
        self.Tdiff = self.Tdiff * 1e-3
        self.bValue = self.bValue * 1e-3
    ...
    def sequenceRun(self, plotSeq=0, demo=False):
        ...
        self.phGradTime = self.phGradTime*1e-3
        self.Tdiff = self.Tdiff*1e-3
        self.tGapDiff2Deph = self.tGapDiff2Deph*1e-3
        self.bValue = self.bValue
        ...
        # Difussion
        # self.ampGdiff = np.sqrt(self.mapVals['bValue'] * 1e6 /( (2 * np.pi * hw.gammaB) ** 2 * ( (self.Tdiff**2 * (self.echoSpacing/2 - self.Tdiff/3)) - gradRiseTime**3 / 30 )))
        self.ampGdiff = np.sqrt(self.mapVals['bValue'] * 1e6/((2 * np.pi * hw.gammaB * self.Tdiff) ** 2 * (self.echoSpacing / 2 - self.Tdiff / 3)))

        self.mapVals['ampGdiff'] = self.ampGdiff
        print(f"|Gdiff| = {1000*self.ampGdiff:.2f} mT/m")

        direction = np.array(self.mapVals['DiffDirection'], dtype=float)
        direction /= np.linalg.norm(direction)
        self.Gdiff = self.mapVals['ampGdiff'] * direction

        print(f"Gdiff = ({1e3*self.Gdiff[0]:.1f}, {1e3*self.Gdiff[1]:.1f}, {1e3*self.Gdiff[2]:.1f}) mT/m")
        tDiff = self.Tdiff * 1e6
        tGapDiff2Deph = self.tGapDiff2Deph * 1e6
        ...
        def createSequence(phIndex=0, slIndex=0, repeIndexGlobal=0, rewrite=True):
            ...
            while acqPoints+self.etl*nRD<=hw.maxRdPoints and orders<=hw.maxOrders and repeIndexGlobal<nRepetitions:
                ...
                if (repeIndex==(self.dummyPulses-1)  or repeIndex>=self.dummyPulses) and dc==False:
                    if self.bValue  > 0:
                        if preemphasisOn == 0:
                            ...
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[0], gSteps, 0, self.shimming)
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[1], gSteps, 1, self.shimming)
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[2], gSteps, 2, self.shimming)
                            orders = orders+gSteps*6

                        if preemphasisOn == 1:
                            t0 = tEx+self.rfExTime/2-hw.gradDelay
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[0], Npointswaveform, 0, self.shimming, AmpEddys, TauEddys)
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[1], Npointswaveform, 1, self.shimming, AmpEddys, TauEddys)
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[2], Npointswaveform, 2, self.shimming, AmpEddys, TauEddys)
                            orders = orders+gSteps*Npointswaveform

                        t0 = tEx+self.rfExTime/2-hw.gradDelay + 2*gradRiseTime + tDiff + tGapDiff2Deph
                ...
                for echoIndex in range(self.etl):
                    ...
                    if self.bValue > 0:
                        if preemphasisOn == 0 and echoIndex==0:
                            t0 = tEcho-self.echoSpacing/2 + self.rfReTime/2-hw.gradDelay
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[0], gSteps, 0, self.shimming)
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[1], gSteps, 1, self.shimming)
                            self.gradTrap(t0, gradRiseTime, tDiff, self.Gdiff[2], gSteps, 2, self.shimming)
                        ...
                        if preemphasisOn == 1 and echoIndex==0:
                            t0 = tEcho-self.echoSpacing/2 + self.rfReTime/2-hw.gradDelay
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[0], Npointswaveform, 0,
                                                     self.shimming, AmpEddys, TauEddys)
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[1], Npointswaveform, 1,
                                                     self.shimming, AmpEddys, TauEddys)
                            self.gradTrapPreemphasis(t0, gradRiseTime, tDiff, self.Gdiff[2], Npointswaveform, 2,
                                                     self.shimming, AmpEddys, TauEddys)
                            orders = orders + gSteps * Npointswaveform

                        # Dephasing phase and slice gradients
                        if repeIndex>=self.dummyPulses:         # This is to account for dummy pulses
                            t0 = tEcho-self.phGradTime - self.rdGradTime/2-3*gradRiseTime-hw.gradDelay
                            self.gradTrap(t0, gradRiseTime, self.phGradTime, phGradients[phIndex], gSteps, self.axesOrientation[1], self.shimming)
                            self.gradTrap(t0, gradRiseTime, self.phGradTime, slGradients[slIndex], gSteps, self.axesOrientation[2], self.shimming)
                            orders = orders+gSteps*4
        ...
        while repeIndexGlobal<nRepetitions:
            nBatches += 1
            if not demo:
                self.expt = ex.Experiment(lo_freq=self.mapVals['larmorFreq'], rx_t=samplingPeriod, init_gpa=init_gpa, gpa_fhdo_offset_time=(1 / 0.2 / 3.1))
