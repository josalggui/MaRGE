...
class RAREProtocolspepeAdPulses(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(RAREProtocolspepeAdPulses, self).__init__()
        # Input the parameters
        self.addParameter(key='seqName', string='RAREInfo', val='RAREprotocolspepeAdPulses')
        self.addParameter(key='toMaRGE', string='to MaRGE', val=True)
        self.addParameter(key='nScans', string='Number of scans', val=1, field='IM')
        self.addParameter(key='larmorFreq', string='Larmor frequency (MHz)', val=3.51, field='RF')
        self.addParameter(key='rfExAmp', string='Excitation RF amp', val=0.2, field='RF')
        self.addParameter(key='rfReAmp', string='Refocusing RF amp', val=0.2, field='RF')
        ...
        self.addParameter(key='factorInvTime', string='Factor inversion time', val=0.5, field='SEQ')
        self.addParameter(key='factorInvGrad', string='Factor inversion gradient', val=0.0, field='SEQ')
        ...
        self.addParameter(key='slicethickness', string='Slice thickness (mm)', val=5, field='IM')
        ...
        self.addParameter(key='PulseShape', string='Pulse shape', val=0, field='RF',
                          tip="0=Rect, 1=Sinc, 2=Sinc+Hanning, 3=HypSec, 4=Chirp, 5=")
    ...
    def sequenceAtributes(self):
        ...
        sliceThickness = self.slicethickness * 1e-3
    ...
    def sequenceRun(self, plotSeq=0, demo=False):
        ...
        rfExAmp = self.mapVals['rfExAmp'] 
        rfReAmp = self.mapVals['rfReAmp']
        factorInvGrad = self.mapVals['factorInvGrad']
        factorInvTime = self.mapVals['factorInvTime']
        ...
        def createSequence(phIndex=0, slIndex=0, repeIndexGlobal=0, rewrite=True):
            ...
            mu = self.mapVals['mu']
            sliceThickness = self.mapVals['slicethickness']*1e-3
            ...
            while acqPoints+self.etl*nRD<=hw.maxRdPoints and orders<=hw.maxOrders and repeIndexGlobal<nRepetitions:
                ...
                if repeIndex>=self.dummyPulses and self.inversionTime!=0:
                    ...
                    self.gradTrap(t0 + hw.blkTime + self.rfReTime, gradRiseTime, self.inversionTime * factorInvTime, 0.005* factorInvGrad, gSteps, self.axesOrientation[0], self.shimming)
                    self.gradTrap(t0 + hw.blkTime + self.rfReTime, gradRiseTime, self.inversionTime * factorInvTime, 0.005* factorInvGrad, gSteps, self.axesOrientation[1], self.shimming)
                    self.gradTrap(t0 + hw.blkTime + self.rfReTime, gradRiseTime, self.inversionTime * factorInvTime, 0.005* factorInvGrad, gSteps, self.axesOrientation[2], self.shimming)
                    orders = orders + gSteps * 6
                ...
                if ShapeEnvelope == 1:
                    self.rfSincPulseWithoutHanning(t0, self.rfExTime, rfExAmp,  int(math.floor(self.rfExTime / undersampling)), rfPhase=0, nLobes=7, channel=0, rewrite=True)

                if ShapeEnvelope == 2:
                    self.rfSincPulseWithHanning(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime/ undersampling)), rfPhase=0, nLobes=7, channel=0, rewrite=True)

                if ShapeEnvelope == 3:
                    self.rfHyperbolicSecantPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime/ undersampling)), beta, mu, rfPhase=0, channel=0, rewrite=True)

                if ShapeEnvelope == 4:
                    self.rfChirpPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime/ undersampling)), beta, rfPhase=0, channel=0, rewrite=True)

                if ShapeEnvelope == 5:
                    self.rfAdiabaticConstantBeff(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime/ undersampling)), beta, rfPhase=0, channel=0, rewrite=True)

                if ShapeEnvelope == 6:
                    self.SliceSelectiveSincPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime/ undersampling)), gradRiseTime, self.mapVals['gSteps'] , sliceThickness, 1, rfPhase=0, nLobes=7, channel=0, gAxis=2, shimming=[0, 0, 0], rewrite=True)

                # Dephasing readout
                if (repeIndex==(self.dummyPulses-1)  or repeIndex>=self.dummyPulses) and dc==False:
                    t0 = tEx+4*self.rfExTime/2-hw.gradDelay
                    # self.rfRecPulse(t0,self.rfExTime,rfExAmp,0,1)
                    # self.rfRecPulse(t0,2*gradRiseTime + self.rdDephTime,0.5,0)
                ...
                for echoIndex in range(self.etl):
                    ...
                    if ShapeEnvelope == 1:
                        self.rfSincPulseWithoutHanning(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), np.pi / 2, nLobes=7, channel=0, rewrite=True)

                    if ShapeEnvelope == 2:
                        self.rfSincPulseWithHanning(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), np.pi / 2, nLobes=7, channel=0, rewrite=True)

                    if ShapeEnvelope == 3:
                        self.rfHyperbolicSecantPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), beta, mu, np.pi / 2, channel=0, rewrite=True)

                    if ShapeEnvelope == 4:
                        self.rfChirpPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), beta, np.pi / 2, channel=0, rewrite=True)

                    if ShapeEnvelope == 5:
                        self.rfAdiabaticConstantBeff(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), beta, np.pi / 2, channel=0, rewrite=True)

                    if ShapeEnvelope == 6:
                        self.SliceSelectiveSincPulse(t0, self.rfExTime, rfExAmp, int(math.floor(self.rfExTime / undersampling)), gradRiseTime, self.mapVals['gSteps'], sliceThickness, 0, np.pi / 2, nLobes=7, channel=0, gAxis=2, shimming=[0, 0, 0], rewrite=True)
                    ...
                    if (repeIndex==(self.dummyPulses-1) or repeIndex>=self.dummyPulses) and dc==False:         # This is to account for dummy pulses
                        ...
                        # self.rfRecPulse(t0, 2*gradRiseTime + self.rdGradTime, 0.5,0)
        ...
        while repeIndexGlobal<nRepetitions:
            ...
            for ii in range(self.nScans):
                if not demo:
                    ...
                    print(f"Batch {nBatches}, Scan {ii} runing... ")
                    data = dataA
