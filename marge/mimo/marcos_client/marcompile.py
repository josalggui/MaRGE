...
def csv2bin(
    ...
    trig_wait_time: nonzero values create a trigger instruction with an adjustable wait time in cycles.
    """

    # Input: CSV column, starting from 0 for tx0 i and ending with either 27 for
    # leds if 0.2-formatted CSV or 33 for third local-oscillator reset if
    # 0.3-formatted CSV
    #
...
def cl2bin(
    ...
    changelist_grad : for the outputs used to control hardware with non-trivial internal timing behaviour
    (currently only the gradient boards)

    initial_bufs: non-default initial values to program the buffers to

    trig_wait_time: nonzero values will add a trigger instruction at the start
    of execution, with an adjustable trigger wait (number of clock cycles to
    wait for a trigger event before giving up and proceeding as if
    triggered). Value must be 24-bit (i.e. < 16,777,215, or ~100ms) for the
    trigger-timeout mode. NOTE: the number has a small offset; if a nonzero
    value is used (e.g. 1) the actual number of cycles before a trigger event
    will be registered and execution will proceed is around 4-5. If a negative
    number is used, then the trigger instruction will block forever until a
    trigger is received.
    """
    ...
    if trig_wait_time < 0:
        bdata.append(insta(ITRIGFOREVER, 0))
    elif trig_wait_time > 0:
        bdata.append(insta(ITRIG, trig_wait_time))
