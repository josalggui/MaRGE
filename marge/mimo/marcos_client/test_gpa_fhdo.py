...
if __name__ == "__main__":
    ...
    dev = Device(samples=params['readout_number'],
        lo_freq=lo_freq,
        tx_t=tx_t,
		rx_t=params['rx_t'],
    ...
    dev.gradb.calibrate(max_current = 2,
    ...
    for ch in range(num_grad_channels):
        dac_code = dev.gradb.ampere_to_dac_code(current)
        dac_code = dev.gradb.calculate_corrected_dac_code(ch,dac_code)
        dev.gradb.write_dac(ch, dac_code)

    wait = input('All gradient channels are set to {:f} A. Press Enter to continue.'.format(current))

    data = dev.run() # Comment out this line to avoid running on the hardware

    # set all channels back to 0 A
    for ch in range(num_grad_channels):
        dac_code = dev.gradb.ampere_to_dac_code(0)
        dac_code = dev.gradb.calculate_corrected_dac_code(ch,dac_code)
        dev.gradb.write_dac(ch, dac_code)
