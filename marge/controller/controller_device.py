...
import marge.mimo.marcos_client.device as dev
import marge.mimo.marcos_client.server_comms as sc
import marge.mimo.marcos_client.mimo_devices as mimo_devices
...
class Experiment(dev.Device):
    ...
    Args:
        ...
        port: Port of SDRLab
        fpga_clk_freq_MHz (float): SDRLab's clock frequency
        grad_board (string): Gradient board of SDRLab; either "ocra1", "gpa-fhdo" or "none".
        ...
        grad_latency: # us, delay inherent in gradient board + external hardware (positive values are corrected for by shifting the gradient update events back in time)
        ...
        trig_output_time (float): When should the master's trig_out pulse occur
        slave_trig_latency (float): How much to delay MIMO master events after the trigger is sent to slaves. Only used for MIMO masters, MIMO slaves or standalone devices unaffected. Device, trigger method and cable-specific.
        trig_timeout (float):  If MIMO slave or externally-triggered master, nonzero values will add a wait-for-trigger instruction to the start of the sequence, with a timeout in microseconds. Positive values must be below 136533 (0.13sec), which is the current limit of marga. Negative values will lead to an infinite wait (i.e. never timing out, blocking forever until a trigger arrives.)
