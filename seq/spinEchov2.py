...
#                    ):
    ...
    def rf_wf(tstart, echo_idx):
        rx_tstart=tstart + (echo_duration - readout_duration)/2
        rx_tend=tstart + (echo_duration + readout_duration)/2
        ...
        if echo_idx == 0:
            ...
            return np.array([tstart + (echo_duration - rf_pi2_duration)/2, tstart + (echo_duration + rf_pi2_duration)/2,
                             tstart + echo_duration - rf_pi_duration/2, rx_tcentre - 10, rx_tcentre + 10]), np.array([pi2_phase*rf_amp, 0, pi_phase*rf_amp, dbg_sc*(1 + 0.5j)])                        
