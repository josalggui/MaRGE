...
class RarePyPulseq(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        ...
        self.addParameter(key='seqName', string='RAREInfo', val='Rare Outer B')
        ...
        self.addParameter(key='parFourierFraction', string='Partial fourier fraction', val=1.0, tip="Fraction of k planes aquired in slice direction")
        ...
        self.addParameter(key='superresolution', string='Multi-contrast super-resolution', val=1, field='PRO',
                          tip='Perform multi-contrast super-resolution: 0 -> do not apply, 1 -> apply. It requires'
                              ' previous run of Rare Outer A properly configuration.')
    ...
    def sequenceAnalysis(self, mode=None):
        ...
        # Average data
        data_rare_outer_b = np.average(data_full, axis=0)

        # Get data from Rare Outer A
        data_rare_outer_a = self.analyze_rare_outer_a(ind_krd_0)

        # Get full data
        data = np.zeros(shape=(self.nPoints0[2], self.nPoints0[1], self.nPoints0[0]), dtype=complex)
        ph_start = (self.nPoints0[1] - self.nPoints[1]) // 2
        ph_end = ph_start + self.nPoints[1]
        sl_start = (self.nPoints0[2] - self.nPoints[2]) // 2
        sl_end = sl_start + self.nPoints[2]
        data[sl_start:sl_end, ph_start:ph_end, :] = data_rare_outer_b
        data = data + data_rare_outer_a
        data = np.reshape(data, newshape=(1, self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2]))

        # Fix the position of the sample according to dfov
        bw = self.getParameter('bw_MHz')
        time_vector = np.linspace(-self.nPoints0[0] / bw / 2 + 0.5 / bw, self.nPoints0[0] / bw / 2 - 0.5 / bw,
                                       self.nPoints0[0]) * 1e-6  # s
        kMax = np.array(self.nPoints0) / (2 * np.array(self.fov)) * np.array(self.mapVals['axes_enable'])
        kRD = time_vector * hw.gammaB * self.getParameter('rd_grad_amplitude')
        kPH = np.linspace(-kMax[1], kMax[1], num=self.nPoints0[1], endpoint=False)
        kSL = np.linspace(-kMax[2], kMax[2], num=self.nPoints0[2], endpoint=False)
        kPH, kSL, kRD = np.meshgrid(kPH, kSL, kRD)
        kRD = np.reshape(kRD, newshape=(1, self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2]))
        kPH = np.reshape(kPH, newshape=(1, self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2]))
        kSL = np.reshape(kSL, newshape=(1, self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2]))
        dPhase = np.exp(2 * np.pi * 1j * (self.dfov[0] * kRD + self.dfov[1] * kPH + self.dfov[2] * kSL))
        data = np.reshape(data * dPhase, newshape=(self.nPoints0[2], self.nPoints0[1], self.nPoints0[0]))
        ...
        # Create sampled data
        kRD = np.reshape(kRD, newshape=(self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2], 1))
        kPH = np.reshape(kPH, newshape=(self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2], 1))
        kSL = np.reshape(kSL, newshape=(self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2], 1))
        data = np.reshape(data, newshape=(self.nPoints0[0] * self.nPoints0[1] * self.nPoints0[2], 1))
        ...
        data = np.reshape(data, newshape=(self.nPoints0[2], self.nPoints0[1], self.nPoints0[0]))
    ...
    def analyze_rare_outer_a(self, ind_krd_0):
        # Load data from Rare Outer A
