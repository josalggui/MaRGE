...
#        self.clearPlotviewLayout()
        ...
        plotSeq = self.plotSeq
        
        if self.sequence.seq == 'R':
            self.tx0_t, self.tx0_y,  self.grad_y_t_float, self.grad_y_a_float, self.grad_z_t_float, self.grad_z_a_float = radial(self.sequence, plotSeq)     
        elif self.sequence.seq == 'GE':
            self.tx0_t, self.tx0_y,  self.grad_x_t_float, self.grad_x_a_float, self.grad_y_t_float, self.grad_y_a_float, self.grad_z_t_float, self.grad_z_a_float = grad_echo(self.sequence, plotSeq)   
        elif self.sequence.seq == 'TSE':
            self.tx0_t, self.tx0_y,  self.grad_x_t_float, self.grad_x_a_float, self.grad_y_t_float, self.grad_y_a_float, self.grad_z_t_float, self.grad_z_a_float = turbo_spin_echo(self.sequence, plotSeq)    
