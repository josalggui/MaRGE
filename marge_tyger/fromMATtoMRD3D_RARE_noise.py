...
def matToMRD(input_path, output_file):
    # --- preparar salida ---
    if output_file is None:
        raise ValueError("Debes proporcionar 'output_file' (ruta o file-like binario).")
    ...
    e.matrix_size = mrd.MatrixSizeType(x=nXYZ[2], y=nXYZ[1], z=nXYZ[0])
    e.field_of_view_mm = mrd.FieldOfViewMm(x=fov[2], y=fov[1], z=fov[0])

    r = mrd.EncodingSpaceType()
    r.matrix_size = mrd.MatrixSizeType(x=nXYZ[2], y=nXYZ[1], z=nXYZ[0])
